...
const ferraris = obj => obj.filter( car => car.brand === 'Ferrari')
// console.log(ferraris(cars))

// 2. Create a function `nineties` that returns all the cars with models between 1980 and 1990.
const nineties = obj => obj.filter( car => car.model >= 1980 && car.model <= 1990)
// console.log(nineties(cars))

// 3. Create a function `list` that returns an array of strings with the same number of cars. Each element should contain the string “This <brand> from <model> costs $<price>” (e.g. for the first element it should say “This Mazda from 1989 costs $5241”).
const list = obj => obj.map( car => `This ${car.brand} from ${car.model} costs $${car.price}`)
// console.log(list(cars))

// 3. Create a function `bmwSum` that returns the sum of all prices of the BMW cars.
const bmwSum = obj => obj.filter(car => car.brand === 'BMW').map( car => car.price).reduce( (acc, cur) => acc + cur, 0)
